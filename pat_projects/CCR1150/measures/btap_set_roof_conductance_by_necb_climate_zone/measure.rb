...
class BTAPSetRoofConductanceByNecbClimateZone < OpenStudio::Ruleset::ModelUserScript
 ...
 # values for option 1
  ...
  def name
    return "BTAPSetRoofConductanceByNecbClimateZone "
  end

  def description
    return "Modifies roof conductances by climate zone."
  end
  # human readable description of modeling approach
  def modeler_description
    return "Modifies roof conductances by climate zone. OpenStudio 2.4.3 (Aug 2018)"
  end
  ...
  def run(model, runner, user_arguments)
    ...
    if  data['hdd18'] <3000 then
      ...
      puts "Cooldn't find a climate zone "
    end
...
# nothing will be done if there are no exterior surfaces
    ...
    final_constructions_array.each do |final_construction|
      ...
      # test if the conductance has changed
       puts "===================== final_conductance_ip is : #{final_conductance_ip} =========================="
...
BTAPSetRoofConductanceByNecbClimateZone.new.registerWithApplication
