      ...
      return false
		...
		model.getSpaceTypes.each do |space_type|
			if space_type.spaces.size > 0
				...
				if !space_type.name.get.to_s.include? "undefined" 
					space_type_to_edits_hash[space_type] = true 
					runner.registerInfo("Applying GSHP system, to space type #{space_type.name.get.to_s}.")
				end
			end		
		end
